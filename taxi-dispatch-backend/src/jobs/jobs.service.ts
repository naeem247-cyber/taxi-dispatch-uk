...
export class JobsService {
  constructor(
    ...
    private readonly logger?: PinoLogger, // optional
  ) {
 ...
 async create(dto: CreateJobDto) {
    ...
    this.logger?.info(
      { job_id: saved.id, status: saved.status },
      'job created',
    );
  ...
  async assign(jobId: string, driverId?: string) {
    return this.dataSource.transaction(async (manager) => {
      ...
      this.logger?.info(
        { job_id: savedJob.id, driver_id: driver.id },
        'job assigned',
      );
  ...
  async updateStatus(
    ...
    if (requester.role === 'driver') {
      if (
        ...
        !job.assignedDriver ||
        job.assignedDriver.accountId !== requester.userId
      ) {
        throw new ForbiddenException(
          'Driver can only update own assigned jobs',
        );
      }
    }
  ...
  private async getSpecificAssignableDriver(
  ...
  private async findNearestAssignableDriver(
    driverRepo: Repository<Driver>,
    job: Job,
  ): Promise<Driver> {
    if (job.pickupLatitude == null || job.pickupLongitude == null) {
      throw new BadRequestException(
        'Job is missing pickup coordinates',
      );
    }
